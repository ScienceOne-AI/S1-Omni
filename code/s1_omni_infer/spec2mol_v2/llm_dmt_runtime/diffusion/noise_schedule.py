...
if __name__ == "__main__":
    ...
    # discrete_sch = NoiseScheduleVP('discrete', betas=0.1)
    poly_sch = NoiseScheduleVP('discrete_poly')
    # t1 = torch.tensor(0.1)
    # t2 = torch.tensor(0.2)
    # t3 = torch.tensor(0.5)
    # t4 = torch.tensor(0.9)
    time_steps = [torch.tensor(0.1), torch.tensor(0.2), torch.tensor(0.3), torch.tensor(0.4), torch.tensor(0.5),
                  torch.tensor(0.6), torch.tensor(0.7), torch.tensor(0.8), torch.tensor(0.9)]

    # for sch in [lin_sch, cos_sch, discrete_sch, poly_sch]:
    for sch in [lin_sch, cos_sch, poly_sch]:
        print(sch.schedule)
        for tt in time_steps:
            print(tt, sch.marginal_prob(tt))
