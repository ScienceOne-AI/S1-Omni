...
class DMT(nn.Module):
    ...
    def build_condition_embedding(self, noise_level, context=None, motif_onehot=None):
        time_emb = self.time_mlp(noise_level)
        if context is not None:
            context = self.cond_encoder(context) # [B, 1, L] -> [B, hidden_dim]
            context = self.cond_lin(context)  # [B, time_dim]
            time_emb = time_emb + context
        return time_emb
    ...
    def forward(self, t, xh, node_mask, edge_mask, context=None, *args, **kwargs):
        ...
        if self.cond_time:
            ...
            motif_onehot = kwargs.get('motif_onehot', None)
            time_emb = self.build_condition_embedding(noise_level, context, motif_onehot)
...
@utils.register_model(name='MAST_DMT')
class MAST_DMT(DMT):
    """Oracle-MAST DMT that fuses known motif one-hot features into time embeddings."""

    def __init__(self, config):
        super().__init__(config)
        motif_dim = getattr(config.model, 'motif_dim', 103)
        motif_hidden_dim = getattr(config.model, 'motif_hidden_dim', 256)
        motif_dropout = getattr(config.model, 'motif_dropout', 0.1)

        self.motif_mlp = nn.Sequential(
            nn.Linear(motif_dim, motif_hidden_dim),
            nn.SiLU(),
            nn.Dropout(motif_dropout),
            nn.Linear(motif_hidden_dim, self.time_dim),
        ...
        nn.init.zeros_(self.motif_gate[0].weight)
        nn.init.constant_(self.motif_gate[0].bias, 4.0)

    def build_condition_embedding(self, noise_level, context=None, motif_onehot=None):
        base_emb = super().build_condition_embedding(noise_level, context, motif_onehot=None)
        if motif_onehot is None:
            return base_emb

        motif_onehot = motif_onehot.to(device=base_emb.device, dtype=base_emb.dtype)
        motif_emb = self.motif_mlp(motif_onehot)
        gate = self.motif_gate(torch.cat([base_emb, motif_emb], dim=-1))
        return gate * base_emb + (1. - gate) * motif_emb
