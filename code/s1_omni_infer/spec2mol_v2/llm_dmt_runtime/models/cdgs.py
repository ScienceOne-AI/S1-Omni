...
class CDGS(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.cond_encoder = SpecFormer(patch_len=config.model.patch_len, stride=config.model.stride, output_dim=nf, spectra_version=config.data.spectra_version)
        self.cond_lin = nn.Linear(nf, nf)
    ...
    def forward(self, t, x, node_mask, edge_mask, context=None, *args, **kwargs):
        ...
        if self.conditional:
            ...
            m_idx += 1
            temb = modules[m_idx](self.act(temb)) + context
            m_idx += 1
        else:
            temb = None
