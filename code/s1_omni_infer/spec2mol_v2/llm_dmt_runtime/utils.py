...
def load_pretrained_checkpoint_compatible(ckpt_dir, state, device,
                                          allowed_missing_prefixes=(),
                                          allowed_unexpected_prefixes=()):
    """Load pretrained model weights while allowing explicitly new modules."""
    if not ckpt_dir:
        return state
    if not os.path.exists(ckpt_dir):
        raise FileNotFoundError(f"Pretrained checkpoint not found: {ckpt_dir}")

    loaded_state = torch.load(ckpt_dir, map_location=device)
    model_state = loaded_state['model'] if 'model' in loaded_state else loaded_state
    incompatible = state['model'].load_state_dict(model_state, strict=False)
    ...
    if bad_missing or bad_unexpected:
        raise RuntimeError(
            ...
            f"Bad missing keys: {bad_missing}; bad unexpected keys: {bad_unexpected}"
        )

    logging.info("Loaded compatible pretrained checkpoint from %s", ckpt_dir)
    logging.info("Allowed missing keys: %s", missing)
    if unexpected:
        logging.info("Allowed unexpected keys: %s", unexpected)
    return state
