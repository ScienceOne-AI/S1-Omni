# ==================================================
# 功能说明：提供 ESM 模型加载与序列编码工具，并优先复用本地缓存权重避免重复下载。
# 使用方法：由训练或推理流程自动调用；可通过环境变量 SIMPLEFOLD_TORCH_HUB_DIR 指定 torch hub 缓存目录。
# 依赖环境：Python 3.10+；依赖库 torch、numpy（可使用 pip install torch numpy 安装）。
...
from s1_omni.protein_folding.utils import residue_constants
...
_MODEL_FILE_MAP = {
    "esm2_t6_8M_UR50D": "esm2_t6_8M_UR50D.pt",
    "esm2_t12_35M_UR50D": "esm2_t12_35M_UR50D.pt",
    "esm2_t30_150M_UR50D": "esm2_t30_150M_UR50D.pt",
    "esm2_t33_650M_UR50D": "esm2_t33_650M_UR50D.pt",
    "esm2_t36_3B_UR50D": "esm2_t36_3B_UR50D.pt",
    "esm2_t48_15B_UR50D": "esm2_t48_15B_UR50D.pt",
}


def _torch_hub_candidates():
    # 优先使用用户显式配置的目录，其次使用你当前机器的常用缓存目录，再回退默认目录。
    custom_hub_dir = os.environ.get("SIMPLEFOLD_TORCH_HUB_DIR")
    if custom_hub_dir:
        yield Path(custom_hub_dir).expanduser()
    yield Path(torch.hub.get_dir())


def _load_from_hub_with_local_cache(repo: str, model_name: str):
    model_file = _MODEL_FILE_MAP.get(model_name)
    if model_file is not None:
        for hub_dir in _torch_hub_candidates():
            ckpt_path = hub_dir / "checkpoints" / model_file
            if ckpt_path.exists():
                torch.hub.set_dir(str(hub_dir))
                print(f"检测到本地 ESM 权重，直接从缓存加载：{ckpt_path}")
                break
    return torch.hub.load(repo, model_name)


load_fn = _load_from_hub_with_local_cache
esm_registry = {
