...
from s1_omni.protein_folding.boltz_data_pipeline.feature.pad import pad_dim
from s1_omni.protein_folding.boltz_data_pipeline.feature.symmetry import (
    get_amino_acids_symmetries,
    get_chain_symmetries,
    get_ligand_symmetries,
)
from s1_omni.protein_folding.boltz_data_pipeline.types import Tokenized
from s1_omni.protein_folding.utils.boltz_utils import center_random_augmentation
...
def process_token_features(
    ...
    # resolved_mask 表示每个 token 是否在结构中被解析出来（即在他们的原子列表中有没有Ca原子)
    # disto_mask 表示每个 token 是否有可用的方向信息 （即在他们的原子列表中有没有Cbeta原子）
...
def process_atom_features(
    ...
    for token_id, token in enumerate(data.tokens):
        ...
        # 为每个原子添加标识
        # 告诉模型，这几个原子构成了一个刚体，要一起旋转平移
        ref_space_uid.extend([new_idx] * token["atom_num"])
        # 告诉模型，这几个原子属于哪个residue，计算时特征要加到一起。
        ...
        # 记录每个token的representative atom的索引，具体操作：使用pdb文件中的ref atom的index减去residues的start id，就是相对
        # 位置，然后再加上atom_idx，此处的atom_idx不同于token[‘atom_idx']，这个是指新创建的这个空间
        ...
        if token["atom_num"] < 3 or res_type in ["PAD", "UNK", "-"]:
            ...
            # 添加错误处理：如果关键原子不在ref_atoms中，跳过该token
            try:
                idx_frame_a = const.ref_atoms[res_type].index("N")
                idx_frame_b = const.ref_atoms[res_type].index("CA")
                idx_frame_c = const.ref_atoms[res_type].index("C")
                mask_frame = (
                ...
                # 关键原子不在ref_atoms中或索引超出范围，跳过该token
                idx_frame_a, idx_frame_b, idx_frame_c = 0, 0, 0
                mask_frame = False
        elif (
            ...
            # 对于DNA/RNA，也添加错误处理
            try:
                idx_frame_a = const.ref_atoms[res_type].index("C1'")
                idx_frame_b = const.ref_atoms[res_type].index("C3'")
                idx_frame_c = const.ref_atoms[res_type].index("C4'")
...
class BoltzFeaturizer:
    ...
    def process(
        ...
        # 加工Token级别的特征，eg：氨基酸类型，CHAIN NUMBER
        token_features = process_token_features(data, max_tokens)

        # Compute atom features
        # 加工原子级别特征，eg：（N，CA，C，O的坐标在哪里）
