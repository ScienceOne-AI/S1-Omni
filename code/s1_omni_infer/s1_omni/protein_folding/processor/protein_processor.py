...
import torch
import numpy as np
from s1_omni.protein_folding.utils.esm_utils import (
    af2_idx_to_esm_idx,
    compute_language_model_representations,
    batch_encode_sequences,
)
from s1_omni.protein_folding.utils.boltz_utils import center_random_augmentation as torch_center_random
...
class ProteinDataProcessor:
    ...
    def process_esm(
        ...
        if multiplicity > 1:
            ...
            expected_tokens = num_tokens[0].item() if isinstance(num_tokens, torch.Tensor) else num_tokens[0]
            actual_tokens = true_len.item() if isinstance(true_len, torch.Tensor) else true_len
            
            # 添加详细的debug信息
            if actual_tokens != expected_tokens:
                print(f"[DEBUG] Token length mismatch detected in process_esm (multiplicity > 1):")
                print(f"  Sequence: {sequence[0][:100]}..." if len(sequence[0]) > 100 else f"  Sequence: {sequence[0]}")
                print(f"  Expected tokens (cropped_num_tokens): {expected_tokens}")
                print(f"  Actual tokens (true_mask.sum()): {actual_tokens}")
                print(f"  Difference: {abs(actual_tokens - expected_tokens)}")
                print(f"  Mask length: {mask[0].sum().item()}")
                print(f"  Linker mask length: {linker_mask[0].sum().item()}")
                print(f"  True mask length: {actual_tokens}")
                print(f"  Sequence length: {len(sequence[0])}")
                print(f"  L (res_type.shape[1]): {L}")
            
            assert true_len == num_tokens[0], (
                f"Token length mismatch: expected {expected_tokens} tokens from cropped_num_tokens, "
                f"but got {actual_tokens} tokens from true_mask. "
                f"Sequence length: {len(sequence[0])}, "
                f"Mask sum: {mask[0].sum().item()}, "
                f"Linker mask sum: {linker_mask[0].sum().item()}"
            )
            esm_s = torch.zeros(
            ...
            for i in range(B):
                ...
                expected_tokens = num_tokens[i].item() if isinstance(num_tokens, torch.Tensor) else num_tokens[i]
                actual_tokens = true_len.item() if isinstance(true_len, torch.Tensor) else true_len
                
                # 添加详细的debug信息
                if actual_tokens != expected_tokens:
                    print(f"[DEBUG] Token length mismatch detected in process_esm (batch {i}):")
                    print(f"  Sequence: {sequence[i][:100]}..." if len(sequence[i]) > 100 else f"  Sequence: {sequence[i]}")
                    print(f"  Expected tokens (cropped_num_tokens): {expected_tokens}")
                    print(f"  Actual tokens (true_mask.sum()): {actual_tokens}")
                    print(f"  Difference: {abs(actual_tokens - expected_tokens)}")
                    print(f"  Mask length: {mask[i].sum().item()}")
                    print(f"  Linker mask length: {linker_mask[i].sum().item()}")
                    print(f"  True mask length: {actual_tokens}")
                    print(f"  Sequence length: {len(sequence[i])}")
                    print(f"  L (res_type.shape[1]): {L}")
                
                assert true_len == num_tokens[i], (
                    f"Token length mismatch for batch {i}: expected {expected_tokens} tokens from cropped_num_tokens, "
                    f"but got {actual_tokens} tokens from true_mask. "
                    f"Sequence length: {len(sequence[i])}, "
                    f"Mask sum: {mask[i].sum().item()}, "
                    f"Linker mask sum: {linker_mask[i].sum().item()}"
                )
